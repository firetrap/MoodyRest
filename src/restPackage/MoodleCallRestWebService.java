...
public class MoodleCallRestWebService implements Serializable{
	...
	public static NodeList call(String params) throws MoodleRestException {
		...
		try {
			...
			final HttpURLConnection connection = (HttpURLConnection) getUrl
					.openConnection();
			...
			while ((line = reader.readLine()) != null) {
				...
				if (error) {
					throw new MoodleRestException(line.substring(
							line.indexOf('>') + 1,
			...
			elements = (NodeList) xpath.evaluate("//VALUE", source,
					XPathConstants.NODESET);
			// Used for testing
			if (debug) {
				for (int i = 0; i < elements.getLength(); i++) {
					String parent = elements.item(i).getParentNode()
							.getParentNode().getParentNode().getParentNode()
							.getNodeName();
					if (parent.equals("KEY")) {
						parent = elements.item(i).getParentNode()
					...
					System.out.println("parent=" + parent + " nodeName="
							+ nodeName + " content=" + content);
				}
			}
			connection.disconnect();
		} catch (final XPathExpressionException ex) {
			Logger.getLogger(MoodleCallRestWebService.class.getName()).log(
		...
		if (MoodleCallRestWebService.getToken() != null) {
			data.append(URLEncoder.encode("wstoken", "UTF-8"))
					...
					.append(URLEncoder.encode(
							MoodleCallRestWebService.getToken(), "UTF-8"));
		} else {
			if (MoodleCallRestWebService.getUsername() != null
				...
				data.append(URLEncoder.encode("wsusername", "UTF-8"))
						.append("=")
						.append(URLEncoder.encode(
								MoodleCallRestWebService.getUsername(), "UTF-8"));
				data.append("&"); // Fix by César Martínez
				data.append(URLEncoder.encode("wspassword", "UTF-8"))
						.append("=")
						.append(URLEncoder.encode(
								MoodleCallRestWebService.getPassword(), "UTF-8"));
			} else {
				return null;
			}
		}
	 ...
	 * </p>
	 * <p>
	 * url contains the URL of the REST server service within the Moodle site.<br />
	...
	public NodeList __call(String url, String params)
			throws MoodleRestException {
		NodeList elements = null;
		try {
			final URL getUrl = new URL(url);
