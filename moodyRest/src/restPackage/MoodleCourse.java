...
public class MoodleCourse implements Serializable{
	...
	public void setMoodleCourseField(String nodeName, String content) {
		...
		if (nodeName.equals("visible")) {
			setVisible(Integer.parseInt(content) == 0 ? COURSE_VISIBLE_TO_STUDENTS_NO
					: COURSE_VISIBLE_TO_STUDENTS_YES);
		}
		if (nodeName.equals("hiddensections")) {
		...
		if (nodeName.equals("enablecompletion")) {
			setEnableCompletion(Integer.parseInt(content) == 0 ? COMPLETION_DISABLED
					: COMPLETION_ENABLED);
		}
		if (nodeName.equals("completionstartonenrol")) {
			setCompletionStartOnEnrol(Integer.parseInt(content) == 0 ? COMPLETION_ON_ENROLL_DISABLED
					: COMPLETION_ON_ENROLL_ENABLED);
		}
		if (nodeName.equals("completionnotify")) {
			setCompletionNotify(Integer.parseInt(content) == 0 ? COMPLETION_NOTIFY_DISABLED
